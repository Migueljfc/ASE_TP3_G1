...
void app_main(void) {
    ...
    const char SinWave[Num_Samples] = {
        // Sin wave 
    ...
    const char TriangularWave[Num_Samples] = {
        // Triangular wave
    ...
    const char SquareWave[Num_Samples] = {
        // Square wave
    ...
    // Writing Sin Wave
    ESP_LOGI(TAG, "Write sin wave: ");
    for (int i = 0; i < sizeof(SinWave); i++) {
        // No need for this EEPROM to erase before write.
        ret = spi_eeprom_write(eeprom_handle, i, SinWave[i]);
        ESP_ERROR_CHECK(ret);
    }

    // Reading Sin Wave
    static uint8_t sin[Num_Samples];
    for (int i = 0; i < sizeof(SinWave); i++) {
        ret = spi_eeprom_read(eeprom_handle, i, &sin[i]);
        // test_WaveFormTables[i][j] = tmp;
        ESP_ERROR_CHECK(ret);
    }
    ESP_LOGI(TAG, "Read sin wave: ");

    // Writing Triangular Wave
    ESP_LOGI(TAG, "Write triangular wave: ");
    for (int i = 0; i < sizeof(TriangularWave); i++) {
        // No need for this EEPROM to erase before write.
        ret = spi_eeprom_write(eeprom_handle, i, TriangularWave[i]);
        ESP_ERROR_CHECK(ret);
    }

    // Reading Triangular Wave
    static uint8_t triangular[Num_Samples];
    for (int i = 0; i < sizeof(TriangularWave); i++) {
        ret = spi_eeprom_read(eeprom_handle, i, &triangular[i]);
        // test_WaveFormTables[i][j] = tmp;
        ESP_ERROR_CHECK(ret);
    }
    ESP_LOGI(TAG, "Read triangular wave: ");

    // Writing Square Wave
    ESP_LOGI(TAG, "Write square wave: ");
    for (int i = 0; i < sizeof(SquareWave); i++) {
        // No need for this EEPROM to erase before write.
        ret = spi_eeprom_write(eeprom_handle, i, SquareWave[i]);
        ESP_ERROR_CHECK(ret);
    }

    // Reading Square Wave
    static uint8_t square[Num_Samples];
    for (int i = 0; i < sizeof(SquareWave); i++) {
        ret = spi_eeprom_read(eeprom_handle, i, &square[i]);
        // test_WaveFormTables[i][j] = tmp;
        ESP_ERROR_CHECK(ret);
    }
    ESP_LOGI(TAG, "Read square wave: ");

    static char WaveFormTable[MaxWaveTypes][Num_Samples];

    for (int i = 0; i < Num_Samples; i++) {
        WaveFormTable[0][i] = sin[i];
    }
    for (int i = 0; i < Num_Samples; i++) {
        WaveFormTable[1][i] = triangular[i];
    }
    for (int i = 0; i < Num_Samples; i++) {
        WaveFormTable[2][i] = square[i];
    }
